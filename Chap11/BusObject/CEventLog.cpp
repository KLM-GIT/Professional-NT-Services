//File CEventLog.cpp
//Implements the CEventLog class
		...
		)
{
	...
	{
		if( szString != NULL ) 
			wsprintf( szErrorDesc, _T( "Function: %s returned Win32 Error: %d, Unknown Error" ), szString, dwErrorNum);
		else
			wsprintf( szErrorDesc, _T( "Win32 Error: %d, Unknown Error" ), dwErrorNum );
	}
	else
	{
		if( szString != NULL ) 
			wsprintf( szErrorDesc, _T( "Function: %s returned Win32 Error: %d Description: %s" ), szString, dwErrorNum, lpvMsgBuf );
		else
			wsprintf( szErrorDesc, _T( "Win32 Error: %d Description: %s" ), dwErrorNum, lpvMsgBuf );
		LocalFree( lpvMsgBuf );
	}
...
BOOL CEventLog::RegisterLog( LPTSTR szPath )
{
	...
	TCHAR szKey[256];
	wsprintf(szKey, 
		_T("SYSTEM\\CurrentControlSet\\Services\\EventLog\\Application\\%s"), m_pszSource);
    ...
    if (RegSetValueEx( hk,            
            ...
            REG_EXPAND_SZ,             
            (LPBYTE) szBuf,
            ((_tcslen(szBuf) + 1) * sizeof(TCHAR)) ) != ERROR_SUCCESS )
		return FALSE;        
