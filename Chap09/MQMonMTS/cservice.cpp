...
void CService::PreInit( )
{
	...
	for( int i = 0; i < NUMEVENTS; i++ )
	{
		...
		if ( !m_hEvents[i] )
			ErrorHandler( _TEXT( "CreateEvent" ) ); 
	}
}
...
{
	m_hServiceStatus =	
		RegisterServiceCtrlHandler( m_szName, lpHandlerProc );

	if( !m_hServiceStatus )
	{
		ErrorHandler( _TEXT( "RegisterServiceCtrlHandler" ) );
		return FALSE;
	}
...
void CService::HandlerMember( DWORD dwControl )
{
	...
	case SERVICE_CONTROL_STOP:
		...
		SetEvent( m_hEvents[ CService::EVENTS::STOP ] );
	...
	case SERVICE_CONTROL_PAUSE:
		...
		SetEvent( m_hEvents[ CService::EVENTS::PAUSE ] );
	...
	case SERVICE_CONTROL_CONTINUE:
		if( GetStatus( ) != SERVICE_RUNNING )
		{
			...
			SetEvent( m_hEvents[ CService::EVENTS::CONTINUE ] );
		}
		break;
	...
	case SERVICE_CONTROL_SHUTDOWN:
		...
		SetEvent( m_hEvents[ CService::EVENTS::SHUTDOWN ] );
...
void CService::LaunchWatcherThread( LPTHREAD_START_ROUTINE pfnWTP )
{
	...
	m_hWatcherThread = (HANDLE)_beginthreadex( NULL, 0, 
			(PBEGINTHREADEX_TFUNC)pfnWTP, 
			0, 0, (PBEGINTHREADEX_TID)&tid );

	if( !m_hWatcherThread )
		ErrorHandler( _TEXT( "_beginthreadex" ) ); 
}
...
DWORD CService::WatcherThreadMemberProc( )
{
	...
	while ( bControlWait )
	{
		...
		{
		case CService::EVENTS::STOP: //STOP
			OnStop( );
			bControlWait = FALSE;
			break;

		case CService::EVENTS::PAUSE: //PAUSE
			OnPause( );

			ResetEvent( m_hEvents[ CService::EVENTS::PAUSE ] );
			break;
		
		case CService::EVENTS::CONTINUE: //CONTINUE
			OnContinue( );

			ResetEvent( m_hEvents[ CService::EVENTS::CONTINUE ] );
			break;
		
		case CService::EVENTS::SHUTDOWN: //SHUTDOWN
			OnShutdown( );
...
{		
	...
	else
	{
		...
		{
			ErrorHandler(_TEXT( "CService::SetStatus: Pending statuses require a hint and checkpoint" ), true, true, 0 ); 
		}
	}
	...
	if( !SetServiceStatus( m_hServiceStatus, &ss ))
		ErrorHandler( _TEXT( "SetServiceStatus" ) ); 
}
